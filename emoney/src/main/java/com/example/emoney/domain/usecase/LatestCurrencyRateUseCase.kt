...
class LatestCurrencyRateUseCase @Inject constructor(
...
) : RemoteUseCase<LatestCurrencyQuery, LatestCurrencyRateResponse, List<LatestCurrencyRate>>(
    errorUtil
) {
    public override suspend fun executeOnBackground(parameters: LatestCurrencyQuery): LatestCurrencyRateResponse {
    ...
    public override suspend fun convert(dto: LatestCurrencyRateResponse): List<LatestCurrencyRate> {
        return mapper.convert(dto.result)
    }
}